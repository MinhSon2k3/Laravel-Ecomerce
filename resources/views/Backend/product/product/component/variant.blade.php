<div class="ibox variant-box">
    ...
    <div class="ibox-content">
        ...
        @php
            $variantCatalouge = old('attributeCatalouge', 
                isset($product->attributeCatalouge) 
                ? json_decode($product->attributeCatalouge, true) 
                : []
            );
        @endphp


        <div class="variant-wrapper  {{ (count($variantCatalouge)) ? '' : 'hidden'}} ">
            ...
            <div class="variant-body">
                @if($variantCatalouge && count($variantCatalouge))
                @foreach($variantCatalouge as $keyAttr =>$valAttr)
                <div class="row mb20 variant-item space-top">
                        ...
                        <div class="attribute-catalouge">
                            <select name="attributeCatalouge[]" id="" class="choose-attribute niceSelect">
                                ...
                                <option {{$valAttr == $val->id ? 'selected' : ''}} value="{{$val->id}}">
                                    {{$val->attribute_catalouge_languages->first()->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-8">
                        ...
                        <select name="attribute[{{$valAttr}}][]" multiple data-catid="{{$valAttr}}"
                    ...
                    <div class="col-lg-1">
                        <button type="button" class="remove-attribute btn btn-danger">
                            <svg data-icon="TrashSolidLarge" aria-hidden="true" focusable="false" width="15" height="16"
...
var attribute = '{{ base64_encode(json_encode(
    old('attribute') ?? (isset($product->attribute) 
        ? json_decode($product->attribute, true) 
        : [])
)) }}';

var variant = '{{ base64_encode(json_encode(
    old('variant') ?? (isset($product->variant) 
